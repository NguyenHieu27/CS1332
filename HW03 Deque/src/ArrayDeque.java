...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        ...
        } else if (size == backingArray.length) { //In case of resizing array
            T[] newArr = (T[]) new Object[2 * backingArray.length];
            for (int i = 0; i < backingArray.length ; i++) {
                newArr[i + 1] = backingArray[mod(front + i, backingArray.length)];
            }
            //newArr[newArr.length / 2] = backingArray[size - 1];
            backingArray = newArr;
            front = 0;
            newArr[0] = data;
            System.out.println(backingArray[front]);
        } else {
            front = mod(front - 1, backingArray.length);
            backingArray[front] = data;
        }
    ...
    public void addLast(T data) {
        ...
        if (size == 0) {
            ...
            if (front == 0) {
                newArr[size - 1] = backingArray[size - 1];
            } else {
                newArr[size - 1] = backingArray[mod(front + size, backingArray.length) - 1];
            }
            backingArray = newArr;
            front = 0;
    ...
    public T removeFirst() {
        if (size == 0) {
            throw new NoSuchElementException("Deque has no data, cannot remove from an empty deque.");
        }
